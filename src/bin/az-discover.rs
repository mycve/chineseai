...
fn heldout_search_report(db: &Connection, model: &AzNnue) -> Result<String> {
    let mut statement = db.prepare("SELECT game,moves,fen,baseline,candidate FROM positions JOIN pairs ON positions.id=pairs.position_id WHERE heldout=1 AND accepted=1")?;
    ...
    Ok(format!(
        "heldout_candidate_selected_400={}/{} 1600={}/{}",
...
fn fit(
    ...
    for (name, rows) in [("control", control), ("experiment", experiment)] {
