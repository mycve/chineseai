...
pub(super) fn sparse_policy(tables: &Tensor, indices: &Tensor) -> Result<Tensor> {
    ...
    if terms != TERMS {
        candle_core::bail!("sparse policy expects {TERMS} indices per move, got {terms}")
    }
    tables.apply_op2(
        indices,
        SparsePolicy {
...
impl CustomOp3 for SparsePolicyGrad {
    ...
    fn cpu_fwd(
        ...
        for (output_index, terms) in indices.chunks_exact(TERMS).enumerate() {
...
fn cuda_gradient(
    ...
    let entries = op.batch * op.moves * TERMS;
    let function = device.get_or_load_custom_func(
        "sparse_policy_grad",
        "chineseai_sparse_policy_v1",
        ptx()?,
