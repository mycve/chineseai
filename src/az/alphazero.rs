...
/// Experimental root-action parallelism. Each considered root action owns an
/// independent subtree and receives the same share of the total simulation budget.
pub fn gumbel_root_parallel_search_with_rules_controlled(
    position: &Position,
    rule_history: Option<Vec<RuleHistoryEntry>>,
    root_moves: Option<Vec<Move>>,
    model: &AzNnue,
    ...
    crate::scope_profile!("az.gumbel_root_parallel_search");
    let history = rule_history.unwrap_or_else(|| position.initial_rule_history());
    let mut root_tree = AzTree::new(position.clone(), history.clone(), root_moves, model, limits);
    let root = root_tree.root;
    root_tree.expand(root);
    root_tree.init_gumbel_root(limits.simulations);
    let mut result = root_tree.search_result(0);
    let action_count = result
        .candidates
        .len()
        .min(limits.max_considered_actions.max(1))
        .min(limits.simulations)
        .min(threads.max(1));
    if action_count == 0 {
        return result;
    }
    let per_action = limits.simulations / action_count;
    let actions = result
        .candidates
        .iter()
        .take(action_count)
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        .num_threads(threads.max(1).min(action_count))
        .build()
        .expect("failed to build root-parallel search pool");
    let branches = pool.install(|| {
        actions
            ...
            .map(|(index, &mv)| {
                ...
                child_history.push(child_position.rule_history_entry_after_move(mv));
                child_position.make_move(mv);
                let child = gumbel_search_with_rules_controlled(
                    &child_position,
                    Some(child_history),
                    None,
                    model,
                    AzSearchLimits {
                        ...
                        seed: limits.seed.wrapping_add((index as u64 + 1) * 0x9e37_79b9),
                        gumbel_scale: 0.0,
    ...
    let mut searched_wdl_sum = [0.0; 3];
    let mut depth_sum = 0.0;
    let mut depth_count = 0usize;
    result.search_depth_max = 0;
    result.search_depth_cutoffs = 0;
    for (mv, child) in branches {
        ...
        candidate.visits = child.simulations.min(u32::MAX as usize) as u32;
        let root_wdl = flip_wdl(child.value_wdl);
        candidate.q = wdl_utility(root_wdl, limits.draw_score.clamp(-1.0, 1.0));
        candidate.moves_left = if child.simulations == 0 {
            child.moves_left + 1.0
        } else {
            child
                .candidates
                .iter()
                .map(|candidate| candidate.moves_left * candidate.visits as f32)
                .sum::<f32>()
                / child.simulations as f32
                + 1.0
        };
        for (sum, probability) in searched_wdl_sum.iter_mut().zip(root_wdl) {
            *sum += probability * child.simulations as f32;
        }
        depth_sum += (child.search_depth_avg + 1.0) * child.simulations as f32;
        depth_count += child.simulations;
        result.search_depth_max = result.search_depth_max.max(child.search_depth_max + 1);
        result.search_depth_cutoffs += child.search_depth_cutoffs;
    }
    result.simulations = depth_count;
    result.search_depth_avg = if depth_count == 0 {
        0.0
    } else {
        depth_sum / depth_count as f32
    };
    if depth_count > 0 {
        result.value_wdl = searched_wdl_sum.map(|value| value / depth_count as f32);
        result.value_q = wdl_utility(result.value_wdl, limits.draw_score.clamp(-1.0, 1.0));
        result.value_cp = cp_from_q(result.value_q);
    }
    complete_root_parallel_candidates(&mut result.candidates, result.value_q, limits.q_value_scale);
    result.candidates.sort_by(|left, right| {
        right
            .visits
            .cmp(&left.visits)
            .then_with(|| right.root_score.total_cmp(&left.root_score))
    });
    result.best_move = result
        .candidates
        .iter()
        .filter(|candidate| candidate.visits > 0)
        .max_by(|left, right| left.root_score.total_cmp(&right.root_score))
        .map(|candidate| candidate.mv)
        .or_else(|| result.candidates.first().map(|candidate| candidate.mv));
    result
}

fn complete_root_parallel_candidates(
    candidates: &mut [AzCandidate],
    raw_value: f32,
    q_value_scale: f32,
) {
    let visited_prior = candidates
        .iter()
        .filter(|candidate| candidate.visits > 0)
        .map(|candidate| candidate.prior.max(f32::MIN_POSITIVE))
        .sum::<f32>();
    let weighted_q = if visited_prior > 0.0 {
        candidates
            .iter()
            .filter(|candidate| candidate.visits > 0)
            .map(|candidate| candidate.prior.max(f32::MIN_POSITIVE) * candidate.q)
            .sum::<f32>()
            / visited_prior
    } else {
        raw_value
    ...
    let max_visits = candidates
        ...
        .max()
        .unwrap_or(0) as f32;
    let scale = (50.0 + max_visits) * q_value_scale.max(0.0);
    let mut max_policy_logit = f32::NEG_INFINITY;
    for (candidate, value) in candidates.iter_mut().zip(values) {
        candidate.completed_q = (value - min) / range * scale;
        let policy_logit = candidate.prior.max(f32::MIN_POSITIVE).ln() + candidate.completed_q;
        candidate.root_score = candidate.gumbel + policy_logit;
        max_policy_logit = max_policy_logit.max(policy_logit);
    }
    let total = candidates
        .iter_mut()
        .map(|candidate| {
            candidate.policy = (candidate.prior.max(f32::MIN_POSITIVE).ln()
    ...
    candidates
        .iter_mut()
        .for_each(|candidate| candidate.policy /= total);
}
...
mod tests {
    ...
    #[test]
    fn root_parallel_evenly_splits_the_total_budget() {
        let result = gumbel_root_parallel_search_with_rules_controlled(
            &Position::startpos(),
            None,
            None,
            &AzNnue::random(4, 107),
        ...
        assert!(searched.iter().all(|candidate| candidate.visits == 16));
        assert!(
            (result
                .candidates
                .iter()
