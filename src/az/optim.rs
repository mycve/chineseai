...
pub(super) struct AdamWState {
    ...
    pub(super) board_hidden_m: Vec<f32>,
    pub(super) board_hidden_v: Vec<f32>,
    pub(super) board_hidden_bias_m: Vec<f32>,
    pub(super) board_hidden_bias_v: Vec<f32>,
...
impl AdamWState {
    pub(super) fn new(model: &AzNnue) -> Self {
        Self {
            ...
            board_hidden_m: vec![0.0; model.board_hidden.len()],
            board_hidden_v: vec![0.0; model.board_hidden.len()],
            board_hidden_bias_m: vec![0.0; model.board_hidden_bias.len()],
            board_hidden_bias_v: vec![0.0; model.board_hidden_bias.len()],
    ...
    pub(super) fn matches(&self, model: &AzNnue) -> bool {
        self.input_hidden_m.len() == model.input_hidden.len()
            ...
            && self.board_hidden_m.len() == model.board_hidden.len()
            && self.board_hidden_v.len() == model.board_hidden.len()
            && self.board_hidden_bias_m.len() == model.board_hidden_bias.len()
            && self.board_hidden_bias_v.len() == model.board_hidden_bias.len()
